...
from PyQt6.QtGui import QImage, QGuiApplication, QVector3D, QColor

# 顶点着色器：利用 gl_VertexID 自动计算全屏大三角形，免去顶点缓存(VBO/VAO)上传！
...
class BlackHoleOverlay(QOpenGLWidget):
    def __init__(self, config):
        ...
        # 黑洞成长速度，从配置读取或设为科学缓速值
        self.growth_rate = self.config.get("blackhole_growth_rate", 0.003)
        self.max_scale = 1.0
        ...
        # 动画定时器 (跑满 GPU 硬件加速的 60 FPS 顺滑帧率)
    ...
    def init_window(self):
        # 无边框、不透明背景（利用桌面截图作为背景，规避透明窗口的显卡驱动兼容性崩溃）
        # 置顶、不在任务栏显示、鼠标事件穿透
        self.setWindowFlags(
    ...
    def initializeGL(self):
        # 1. 初始化 OpenGL 2.1 函数指针对象
        profile = QOpenGLVersionProfile()
        profile.setVersion(2, 1)
        self.funcs = QOpenGLVersionFunctionsFactory.get(profile, self.context())
        if not self.funcs:
            print("Failed to get OpenGL version functions in context.")
            return
        self.funcs.initializeOpenGLFunctions()
        
        # 标记 OpenGL 环境已初始化完毕，后续 makeCurrent 才能安全调用！
        self.gl_initialized = True
        
        # 2. 构建片段着色器
        # 注入我们的 uniforms，并采用重定义宏占位 Ghostty 内置变量，防 Undefined 错误
...
uniform int uStyleIndex;
uniform float uDriftSpeed;

#define DRIFT_SPEED uDriftSpeed

// Ghostty custom shader built-ins placeholders
#define iCurrentCursorColor vec4(0.0)
#define iPreviousCursorColor vec4(0.0)
#define iTimeCursorChange 0.0
#define iDate vec4(0.0)
"""
        # 读取本地 blackhole.glsl 算法
        glsl_content = ""
        possible_paths = [
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "blackhole.glsl"),
            os.path.join(os.getcwd(), "blackhole.glsl"),
            "blackhole.glsl"
        ]
        ...
        # 如果读取不到（比如打包环境下且外部文件丢失），使用备用内联 GLSL 字符串
        if not glsl_content:
            glsl_content = self.get_fallback_glsl()
            
        # 适配与接管
        glsl_content = glsl_content.replace("float tokenLevel() {", "float tokenLevel() { return uIntensity; ")
        glsl_content = glsl_content.replace("const float DRIFT_SPEED   = 1.0000;", "// const float DRIFT_SPEED   = 1.0000;")
        
        target = """    float I, sz;
    vec2  center;
    if (SIZE_MODE == MODE_POMODORO) {"""
    
        replacement = """    float I = uIntensity;
    float sz = uRadiusScale;
    vec2  center = uHoleCenter;
    if (false) {"""
        
        glsl_content = glsl_content.replace(target, replacement)
        
        # 将 LOOK_DEFAULT 赋值替换为我们自定义的样式索引读取，支持动态切换 8 种不同的电影级特效样式！
        target_look = """    DiskLook L = LOOK_DEFAULT;
    if (SIZE_MODE == MODE_DEMO) L = demoLook();"""
        replacement_look = """    DiskLook L = DEMO_TOUR[clamp(uStyleIndex, 0, 7)];"""
        glsl_content = glsl_content.replace(target_look, replacement_look)
...
"""
        full_fragment_shader = header + glsl_content + footer
        
        # 3. 编译着色器程序
        self.program = QOpenGLShaderProgram()
        self.program.addShaderFromSourceCode(QOpenGLShader.ShaderTypeBit.Vertex, VERTEX_SHADER_SOURCE)
        self.program.addShaderFromSourceCode(QOpenGLShader.ShaderTypeBit.Fragment, full_fragment_shader)
        if not self.program.link():
            print("OpenGL Shader Linking Error:", self.program.log())
            
        # 4. 创建纹理（在 Context 确实建立完毕后进行）
        if self.screenshot_image:
            self.texture = QOpenGLTexture(self.screenshot_image)
    ...
    def resizeGL(self, w, h):
        if self.funcs:
            self.funcs.glViewport(0, 0, w, h)
            
    def paintGL(self):
        if not self.funcs or not self.program:
            return
            
        funcs = self.funcs
        funcs.glClearColor(0.0, 0.0, 0.0, 1.0)
        funcs.glClear(funcs.GL_COLOR_BUFFER_BIT | funcs.GL_DEPTH_BUFFER_BIT)
        
        self.program.bind()
        
        # 传入状态参数 Uniforms
        w, h = self.width(), self.height()
        self.program.setUniformValue("iTime", float(self.elapsed_time))
        self.program.setUniformValue("iResolution", QVector3D(float(w), float(h), 1.0))
        self.program.setUniformValue("uIntensity", float(self.intensity))
        self.program.setUniformValue("uRadiusScale", float(self.radius_scale))
        self.program.setUniformValue("uHoleCenter", QPointF(self.cx / w, 1.0 - (self.cy / h))) # y轴反转
        self.program.setUniformValue("uStyleIndex", int(self.style_index))
        self.program.setUniformValue("uDriftSpeed", float(self.config.get("blackhole_drift_speed", 1.0)))
        
        # 绑定贴图
        if self.texture:
            funcs.glActiveTexture(funcs.GL_TEXTURE0)
            self.texture.bind()
            self.program.setUniformValue("iChannel0", 0)
            
        # 绘制全屏大三角形
        funcs.glDrawArrays(funcs.GL_TRIANGLES, 0, 3)
    ...
    def start(self):
        ...
        # 1. 随机化黑洞的生成中心与样式样式（每次出现都是不同的特效，免去用户手动设置）
        self.style_index = random.randint(0, 6)
        border_w = int(self.screen_w * 0.2)
        border_h = int(self.screen_h * 0.2)
        self.cx = random.randint(border_w, self.screen_w - border_w)
        self.cy = random.randint(border_h, self.screen_h - border_h)
        ...
        # 3. 截取当前主屏幕作为底层纹理，加入 null 判断防崩溃
        screen = QGuiApplication.primaryScreen()
        pixmap = screen.grabWindow(0)
        if pixmap.isNull():
            # 极少情况下如果抓图失败，创建暗色后备图
            self.screenshot_image = QImage(256, 256, QImage.Format.Format_ARGB32)
            self.screenshot_image.fill(QColor(30, 30, 30))
        else:
            self.screenshot_image = pixmap.toImage().mirrored(False, True)
            
        # 4. 【核心修复】：如果 OpenGL 已经初始化过（后续触发），直接安全更新纹理；
        # 如果是首次触发，OpenGL 上下文尚未建立，切勿调用 makeCurrent()，其数据会被 initializeGL() 自动处理！
    ...
    def stop(self):
        ...
        # 释放 GPU 纹理资源
        if self.gl_initialized and self.texture:
            self.makeCurrent()
            try:
                self.texture.destroy()
            except Exception:
    ...
    def update_frame(self):
        ...
        # 平滑地渐变膨胀黑洞，约 3 秒后长到饱满尺寸
        if self.radius_scale < self.max_scale:
            self.radius_scale += self.growth_rate
            self.intensity = self.radius_scale
        else:
            self.radius_scale = self.max_scale
            self.intensity = self.max_scale
    ...
    def get_fallback_glsl(self):
        return r'''// blackhole.glsl — a geodesic-traced black hole for Ghostty
//
